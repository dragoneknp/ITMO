...
def main():
    ...
    filein=open('multimap.in', 'r')
    fileout=open('multimap.out','w')
    for s in filein.readlines():
        s=s.split()
        if s[0] == 'put':
            add(s[1],s[2],keys)
        elif s[0]=='delete':
            ...
            if a==0:
                a=1
            print(a-1,end=' ',file = fileout)
            print(*b[:-1],file = fileout)
        else:
            deleteall(s[1],keys)
    fileout.close()
main()
